...
export const GithubStarsButton = ({
  ...
  children,
  className,
  repo = "ldblckrs-258/novel-studio",
  ...props
}: Readonly<{
  href: string;
  children?: string;
  ...
  return (
    ...
    >
      ...
      <span
        className="inset-0 absolute pointer-events-none select-none rounded-full overflow-hidden"
        style={{
          animation: "10s ease-in-out infinite alternate border-glow-translate",
        }}
      >
        <span
          className="absolute left-1/2 top-0 block z-0 h-full w-12 blur-xl rounded-full"
          style={{
      ...
      <span className="flex items-center justify-center gap-1 relative z-1 dark:bg-neutral-950/90 bg-neutral-50/90 rounded-full py-2 px-4 pl-2 w-full">
        <span className="relative group-hover:scale-105 transition-transform group-hover:rotate-360">
          <CuicuiStarIcon
          ...
          />
          <span
            className="rounded-full size-11 absolute opacity-0 dark:opacity-30 top-1/2 left-[175%] -translate-x-1/2 blur-lg"
            style={{
